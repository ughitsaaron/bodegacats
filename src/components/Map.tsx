...
export function Map(props: PropsWithChildren) {
  ...
  const initMapCallback = useCallback<RefCallback<HTMLDivElement>>((node) => {
    async function init() {
      try {
        if (node && !map) {
          const L = await import('leaflet');
          const leaflet = new L.Map(node).setView(coords, 12);
          L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png').addTo(leaflet);
          setMap(leaflet);
        }
      } catch {
        // hmm…
  ...
  return (
    <MapProvider value={{ map }}>
      <div style={{ position: 'absolute', width: '100%', height: '100%', left: 0, right: 0, top: 0, bottom: 0, zIndex: 0 }}>
